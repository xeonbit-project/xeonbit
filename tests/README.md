...
Core tests take longer than any other Xeonbit tests, due to the high amount of computational work involved in validating core components.
...
To run only Xeonbit's core tests (after building):
...
To run only Xeonbit's crypto tests (after building):
...
```
xeonbitd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `xeonbit-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Xeonbit's hash tests (after building):
...
To run only Xeonbit's performance tests (after building):
...
To run only Xeonbit's unit tests (after building):
